...
def main():
    ...
    while True:
        ...
        if g == S_BLIND_SELECT:
            ...
            # Respect buy_jokers config flag from mod
            if not gs.config.get("buy_jokers", True):
                print("  → buy_jokers disabled in mod config — skipping purchases")
                nav.leave_shop(joker_slots=[])
