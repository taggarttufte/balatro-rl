...
-- All navigation (blind select, cash out, shop, new run) is handled by Python via pyautogui.

if not BalatroRL then
...
function BalatroRL.write(event)
    local ok, err = pcall(function()
        ...
        if state then love.filesystem.write(STATE_FILE, json_encode(state)) end
    end)
    if not ok then
        pcall(function()
            love.filesystem.createDirectory(STATE_DIR)
...
Game.update = function(self, dt)
    ...
    -- Card action polling (play/discard only)
    pcall(function()
